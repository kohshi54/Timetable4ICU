...
struct EditView: View {
    ...
    var body: some View {
        VStack {
            HStack {
                VStack {
                    if tmpClassInfo.courseTitle.isEmpty {
                        Text("Course Title").foregroundColor(Color("placeholderTextColor"))
                    } else {
            ...
            Group {
                TextField("Instructor", text: $tmpClassInfo.instructor).textFieldStyle(RoundedBorderTextFieldStyle())
                    .padding(.horizontal)
                Group {
                    HStack {
                        VStack {
                            if tmpClassInfo.schedule.isEmpty {
                                Text("Schedule").foregroundColor(Color("placeholderTextColor"))
                            } else {
                    ...
                    .overlay(
                        RoundedRectangle(cornerRadius: 4.0)
                            .stroke(Color("textfield"), lineWidth: 0.5)
                    )
                    .padding(.horizontal)
                    .contentShape(RoundedRectangle(cornerRadius: 4.0))
                    .onTapGesture {
                ...
                TextField("Room", text: $tmpClassInfo.room).textFieldStyle(RoundedBorderTextFieldStyle())
                    .padding(.horizontal)
                Group {
                    HStack {
                        VStack {
                            if tmpClassInfo.mode.isEmpty {
                                Text("Mode").foregroundColor(Color("placeholderTextColor"))
                            } else {
                    ...
                    HStack {
                        VStack {
                            if tmpClassInfo.color.isEmpty {
                                Text("Color").foregroundColor(Color("placeholderTextColor"))
                            } else {
        ...
        .navigationTitle("Edit View in \(cellNameWithSat[hnum][vnum])")
        .toolbar {
            ToolbarItem(placement: .principal) {
                Text("Edit View in \(cellNameWithSat[hnum][vnum])")
                    .font(.headline)
